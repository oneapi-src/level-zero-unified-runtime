...
class Logger {
  public:
    Logger(std::unique_ptr<logger::Sink> sink,
        ...
        : standardSinkLevel(UR_LOGGER_LEVEL_QUIET),
          standardSink(std::move(sink)),
    ...
    template <typename... Args> void error(const char *format, Args &&...args) {
        log(UR_LOGGER_LEVEL_ERR, format, std::forward<Args>(args)...);
    }
    ...
    void error(const logger::LegacyMessage &p, const char *format,
               Args &&...args) {
        log(p, UR_LOGGER_LEVEL_ERR, format, std::forward<Args>(args)...);
    }
    ...
    void log(const logger::LegacyMessage &p, ur_logger_level_t level,
        ...
        if (standardSink) {
            if (isLegacySink) {
                standardSink->log(level, p.message,
                                  std::forward<Args>(args)...);
                return;
            }
    ...
    void setCallbackSink(ur_logger_output_callback_t cb, void *pUserData,
