...
ur_result_t urUSMDeviceAlloc(
  ...
  return getMsanInterceptor()->allocateMemory(hContext, hDevice, pUSMDesc, pool,
                                              size, ppMem);
}
...
ur_result_t UR_APICALL urEnqueueUSMFill(
  ...
  if (phEvent) {
    UR_CALL(getContext()->urDdiTable.Enqueue.pfnEventsWait(hQueue, 2, hEvents,
                                                           phEvent));
  }
