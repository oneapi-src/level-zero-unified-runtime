...
__urdlllocal ur_result_t UR_APICALL urCommandBufferRetainCommandExp(
  ...
  auto dditable =
      reinterpret_cast<ur_exp_command_buffer_command_object_t *>(hCommand)
          ->dditable;
  auto pfnRetainCommandExp = dditable->ur.CommandBufferExp.pfnRetainCommandExp;
...
__urdlllocal ur_result_t UR_APICALL urTensorMapEncodeIm2ColExp(
  ...
  // forward to device-platform
  result = pfnEncodeIm2ColExp(hDevice, TensorMapType, TensorRank, GlobalAddress,
...
UR_DLLEXPORT ur_result_t UR_APICALL urGetTensorMapExpProcAddrTable(
  ...
  for (auto &platform : ur_loader::getContext()->platforms) {
    ...
    if (!getTable)
      continue;
    platform.initStatus = getTable(version, &platform.dditable.ur.TensorMapExp);
  }
  ...
  if (UR_RESULT_SUCCESS == result) {
    if (ur_loader::getContext()->platforms.size() != 1 ||
        ur_loader::getContext()->forceIntercept) {
      // return pointers to loader's DDIs
      pDdiTable->pfnEncodeIm2ColExp = ur_loader::urTensorMapEncodeIm2ColExp;
