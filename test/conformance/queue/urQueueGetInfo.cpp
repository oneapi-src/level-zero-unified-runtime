...
TEST_P(urQueueGetInfoTest, Context) {
    ...
    size_t size = 0;
    auto info_type = UR_QUEUE_INFO_CONTEXT;
    ...
    ASSERT_SUCCESS(
        urQueueGetInfo(queue, info_type, size, &returned_context, nullptr));
...
TEST_P(urQueueGetInfoTest, Device) {
    ...
    size_t size = 0;
    auto info_type = UR_QUEUE_INFO_DEVICE;
    ASSERT_SUCCESS(urQueueGetInfo(queue, info_type, 0, nullptr, &size));
    ASSERT_EQ(sizeof(ur_device_handle_t), size);

    ur_device_handle_t returned_device = nullptr;
    ASSERT_SUCCESS(
        urQueueGetInfo(queue, info_type, size, &returned_device, nullptr));
...
TEST_P(urQueueGetInfoTest, Flags) {
    ...
    size_t size = 0;
    auto info_type = UR_QUEUE_INFO_FLAGS;
    ASSERT_SUCCESS(urQueueGetInfo(queue, info_type, 0, nullptr, &size));
    ASSERT_EQ(sizeof(ur_queue_flags_t), size);

    ur_queue_flags_t returned_flags = 0;
    ASSERT_SUCCESS(
        urQueueGetInfo(queue, info_type, size, &returned_flags, nullptr));
...
TEST_P(urQueueGetInfoTest, ReferenceCount) {
    ...
    size_t size = 0;
    auto info_type = UR_QUEUE_INFO_REFERENCE_COUNT;
    ASSERT_SUCCESS(urQueueGetInfo(queue, info_type, 0, nullptr, &size));
    ASSERT_EQ(sizeof(uint32_t), size);

    uint32_t returned_reference_count = 0;
    ASSERT_SUCCESS(urQueueGetInfo(queue, info_type, size,
...
TEST_P(urQueueGetInfoTest, EmptyQueue) {
    ...
    size_t size = 0;
    auto info_type = UR_QUEUE_INFO_EMPTY;
...
TEST_P(urQueueGetInfoDeviceQueueTestWithInfoParam, DeviceDefault) {
    size_t size = 0;
    auto info_type = UR_QUEUE_INFO_DEVICE_DEFAULT;
    ASSERT_SUCCESS_OR_OPTIONAL_QUERY(
        urQueueGetInfo(queue, info_type, 0, nullptr, &size), info_type);
    ...
    ASSERT_SUCCESS(
        urQueueGetInfo(queue, info_type, size, &returned_queue, nullptr));
...
TEST_P(urQueueGetInfoDeviceQueueTestWithInfoParam, Size) {
    size_t size = 0;
    auto info_type = UR_QUEUE_INFO_SIZE;
    ASSERT_SUCCESS_OR_OPTIONAL_QUERY(
        urQueueGetInfo(queue, info_type, 0, nullptr, &size), info_type);
    ASSERT_EQ(sizeof(uint32_t), size);
    ...
    ASSERT_SUCCESS(
        urQueueGetInfo(queue, info_type, size, &returned_size, nullptr));
