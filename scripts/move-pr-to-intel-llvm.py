...
# TODO: intel_llvm_slug = "intel/llvm"
intel_llvm_slug = "kbenzie/intel-llvm"
...
def main():
    ...
    if args.intel_llvm_remote == args.intel_llvm_push_remote:
        ...
        if not confirm(
            f"Are you sure you want to push directly to {intel_llvm_slug}?"
        ):
            exit(1)
    ...
    if not locally and not remotely:
        ...
        checkout_branch(
            git, args.intel_llvm_dir, args.intel_llvm_feature_branch
        )
...
def check_remote_url(
    git: str, dir, remote: str, slug: str, option: str
) -> None:
    result = run(
        [
            git,
...
def check_ur_branch_is_updated(
    git: str, dir: str, remote: str, branch: str
) -> None:
    rev_list = run(
        [
            git,
